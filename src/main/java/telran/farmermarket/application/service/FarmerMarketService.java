...
public class FarmerMarketService implements IFarmerMarketService {
	...
	public FarmerDto addProductToFarmer(String farmerName, String productName, int quantity) {
		...
		Optional<FarmerProductInfo> productInStock = farmer.getStock().stream()
				.filter(fp -> fp.getProductId().equals(product.getId())).findFirst();

		if (productInStock.isPresent()) {
			FarmerProductInfo fp = productInStock.get();
			fp.setQuantity(fp.getQuantity() + quantity);
		} else {
			farmer.getStock().add(new FarmerProductInfo(product.getId(), product.getName(), quantity));
		}
	...
	public FarmerDto sellProductToClient(String clientEmail, String farmerName, String productName, int quantity) {
		...
		FarmerProductInfo productInStock = farmer.getStock().stream()
				.filter(fp -> fp.getProductId().equals(product.getId())).findFirst()
				.orElseThrow(() -> new RuntimeException("SELL_PRODUCT_TO_CLIENT - PRODUCT_NOT_IN_STOCK"));
	...
	@Override
	public List<FarmerProductDto> getFarmerStock(String farmerName) {
		Farmer farmer = getFarmerByName(farmerName);
		return farmer.getStock().stream().map(fp -> new FarmerProductDto()).collect(Collectors.toList());
	}
